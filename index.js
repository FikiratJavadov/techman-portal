...
import {without, split, keys, juxt} from 'ramda'
...
export const Observable = (init = {}) => {
  ...
  const handler = {
    ...
    set(target, path, value) {
      ...
      Array.from(subscribers.keys()).map(x =>
        new RegExp(x).test(fullPath) && subscribers.get(x).map(f => f(value, path))
      )
  ...
  init.subscribe = (path, fn) => {
    ...
    return () => {
      subscriber = subscribers.get(path).filter(s => s !== fn)
...
export const useObservable = (paths, timeout = 100) => {
  if (!paths) return
